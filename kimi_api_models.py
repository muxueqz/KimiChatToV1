...
from bottle import request, HTTPResponse as Response
import bottle
import requests
import json
...
def create_new_chat_session():
    ...
    # 定义请求的载荷
    payload = {"name": "未命名会话", "is_example": False}

    # 发送POST请求
    response = requests.post(
        "https://kimi.moonshot.cn/api/chat", json=payload, headers=headers
    )
...
def stream_chat_responses_route():
    ...
    if not tokens["refresh_token"]:
        ...
        return bottle.HTTPResponse(data, 404)
                ...
                if line:
                    ...
                    if decoded_line.startswith("data: "):
                        ...
                        try:
                            ...
                            if (
                                "text" in json_obj and json_obj.get("event") == "cmpl"
                            ):  # 检查 'event' 字段的值是否为 'cmpl'
                                # 构造 JSON 对象
                                response_json = {
                                    "choices": [
                                        {
                                ...
                                "text" in json_obj and json_obj.get("event") == "rename"
                            ):
                                ...
                                save_rename_text(
                                    json_obj["text"]
                                )  # 保存 'rename' 事件的 'text' 到 config.yaml 文件
                            elif (
                                json_obj.get("event") == "search_plus"
                            ):  # 检查 'event' 字段的值是否为 'search_plus'
